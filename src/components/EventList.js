...
function EventList({eventsList}) {
  ...
  return (
    <div className='bg-gradient-to-b from-stone-200 to-stone-400'>
        ...
        <span onClick={()=> chooseEventList(0)} className={`duration-300 select-none ${!choice ? 'border-b-2 lg:pb-2 pb-1 border-slate-800 text-slate-800 duration-1000 scale-105' : 'text-lime-900'}`}>
              
              Technical
            </span>
            <span onClick={()=> chooseEventList(1)} className={`duration-300 select-none ${!choice ? 'text-lime-900' : 'border-b-2 lg:pb-2 pb-1 border-slate-800 text-slate-800 duration-1000 scale-105'}`}>
